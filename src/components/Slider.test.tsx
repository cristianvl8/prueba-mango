...
import Slider from './Slider'; // Ajusta esta importación según la estructura de tu proyecto
...
describe('Slider Component', () => {
    ...
    describe('Slider Component - Prompt interaction', () => {
        ...
        beforeEach(() => {
          window.prompt = jest.fn(); // Mockea la función prompt
        });
        ...
        it('should update the min value when click minValue currency and a new valid number is entered', async () => {
            ...
            (window.prompt as jest.Mock).mockReturnValue(newMinValue); // Configura el valor devuelto por el prompt mockeado
            ...
            const minValueElement = screen.getByTestId('min-value');
            await fireEvent.click(minValueElement); // Asume que tienes un data-testid para el trigger  
            expect(minValueElement.textContent).toBe(newMinValue);
        });
            
        afterEach(() => {
            jest.resetAllMocks(); // Restablece los mocks después de cada prueba
        });
